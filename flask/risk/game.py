from risk.display import Display, CursesDisplay
from risk.player import Player
from risk.territory import World
from risk.world import CONNECT, AREAS, MAP, KEY
import logging
...
class Game(object):
    ...
    def aiwarn(self, *args):
        ...
        logging.getLogger(
            "pyrisk.player.%s" % self.player.ai.__class__.__name__
        ).warning(*args)
    ...
    def combat(self, src, target, f_atk, f_move):
        """Simulate a Risk combat round

        Args:
            src (Territory): _description_
            target (Territory): _description_
            f_atk (Callable[[int, int], bool]): _description_
            f_move (Callable[int, int]): _description_
        ...
        if n_def == 0:
            ...
            if move < min_move:
                self.aiwarn(
                    "combat invalid move request %s (%s-%s)", move, min_move, max_move
                )
                move = min_move
            if move > max_move:
                self.aiwarn(
    ...
    def initial_placement(self):
        ...
        if self.options["deal"]:
            ...
            while empty:
                ...
                t.owner = self.player
                self.event(
                    ("deal", self.player, t), territory=[t], player=[self.player.name]
                )
                self.turn += 1
        else:
            while empty:
                ...
                empty.remove(t)
                self.event(
                    ("claim", self.player, t), territory=[t], player=[self.player.name]
                )
                self.turn += 1
