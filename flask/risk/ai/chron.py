...
class ChronAI(AI):
    ...
    def start(self):
        ...
        self.distance = {
            t: len(self.pathfind(self.seed, t, hostile=False, forces=False))
            for t in self.world.territories.values()
        }
        self.area_distance = {
            a: float(sum(self.distance[t] for t in a.territories)) / len(a.territories)
            for a in self.world.areas.values()
        }
        self.area_priority = sorted(
            self.area_distance.keys(), key=lambda x: self.area_distance[x]
        )
    ...
    def initial_placement(self, empty, remaining):
        if empty:
            ...
            for e in empty:
                ...
                score[e] += max(
                    e.area.value
                    - 2 * (len(e.area.territories) - enemy_count.get(e.area, 0)),
                    0,
                )
                score[e] += sum(
                    [1.0 for tt in e.area.territories if tt.owner == self.player]
                ) / len(e.area.territories)
    ...
    def needed_attackers(self, defenders, attackers=1, prob=0.50, survivors=1):
        ...
        while True:
            ...
            if a >= survivors:
                ...
                attackers += 1
        self.loginfo(
            "needed_attackers vs=%s, p=%s, survive=%s-> %s",
            defenders,
            prob,
            survivors,
            attackers,
    ...
    def strategy(self):
        ...
        self.loginfo(
            "strategy: forces=%s reinforcements=%s areas=%s territories=%s",
            self.player.forces,
            self.player.reinforcements,
            tuple(self.player.areas),
            tuple(self.player.territories),
        )

        strength_order = sorted(
            self.game.players.values(), key=lambda x: x.forces + x.reinforcements
        )
        ...
        if self.seed.owner != self.player:
            if have_area:
                self.seed = random.choice(
                    [t for t in self.player.territories if t.area.owner == self.player]
                )
            else:
                has_adjacent = [
                    t
                    for t in self.player.territories
                    if list(t.adjacent(friendly=True))
                ]
                if has_adjacent:
    ...
    def evaluate_attack(self, taken):
        ...
        result["enemy-reinforcements"] = sum(
            p.reinforcements for p in toy_players if p != toy_us
        ) - sum(
        ...
        result["freed-forces"] = sum(
            self.world.territories[t].forces for t in old_border - new_border
        )

        # change of hostiles up against our border - -ve if situation improved
        result["border-hostiles"] = sum(
            sum(a.forces for a in t.adjacent(friendly=False))
            for t in toy_us.territories
            if t.border
        ) - sum(
            sum(a.forces for a in t.adjacent(friendly=False))
            for t in self.player.territories
    ...
    def plan_attack(self, srcs, targets, defenses, prob, tries=100):
        ...
        self.loginfo(
            "plan_attack srcs=%s targets=%s defenses=%s p=%s",
            srcs,
            targets,
            defenses,
            prob,
        ...
        # print(possible)
        defended_possible = list(filter(
            lambda x: set(xx[-1] for xx in x) >= set(defenses.keys()), possible
        ))
        ...
        for p in possible:
            needed[p] = [
                self.needed_attackers(
                    [t.forces for t in r[1:]], r[0].forces, prob, defenses.get(r[0], 1)
                )
                for r in p
            ]
        ...
        # print(possible)
        self.loginfo(
            "plan_attack: route=%s, needed=%s", possible[0], needed[possible[0]]
        )
    ...
    def reinforce(self, available):
        ...
        for t in self.player.territories:
            if t.border:
                ...
                elif self.priority["defend-connected"] > 0 and self.pathfind(
                    t, self.seed, hostile=False
                ):
                    needed = self.needed_reinforcements(
                        t, self.priority["defend-connected"]
                    )
                elif self.priority["defend-isolated"] > 0:
                    needed = self.needed_reinforcements(
                        t, self.priority["defend-isolated"]
                    )
                if needed:
                    result[t] = needed
        ...
        if wanted > available:
            self.loginfo(
                "reinforce: unable to meet defense requirement (%s/%s)",
                wanted,
                available,
            )
        ...
        for pri in ("take-area", "shorten-border", "target-stronger", "target-weaker"):
            if available and self.priority[pri] > 0:
                ...
                if pri == "take-area":
                    for a in self.world.areas.values():
                        if (
                            any(t.owner == self.player for t in a.territories)
                            and not a.owner
                        ):
                            needed = [
                                t for t in a.territories if t.owner != self.player
                            ]
                    ...
                    possible = sorted(
                        possible, key=lambda x: x["resistance"] - x["freed-forces"]
                    )
                    ...
                    for i, a1 in enumerate(adjacent[:-1]):
                        for a2 in adjacent[i + 1 :]:
                            if (
                                a1 not in planned
                                and a2 not in planned
                                and a2 in a1.connect
                            ):
                    ...
                    for a in adjacent:
                        if (
                            a not in planned
                            and a.owner.reinforcements + a.owner.forces
                            > self.player.reinforcements + self.player.forces
                        ):
                            possible.append(self.evaluate_attack([a]))
                    possible = sorted(
                        possible,
                        key=lambda x: x["resistance"] - x["enemy-reinforcements"],
                    )
                ...
                for p in possible:
                    ...
                    plan = self.plan_attack(
                        srcs,
                        taken,
                        {t: int(4 * self.priority[pri]) for t in border},
                        self.priority[pri],
                    )
                    if plan:
                        routes, needed = plan
                        ...
                        if sum(needed) <= remaining and not touched & planned:
                            for r, n in zip(routes, needed):
                                ...
                                remaining -= n
                            self.loginfo(
                                "reinforce: made plan type=%s route=%s needed=%s, now available=%s",
                                pri,
                                routes,
                                needed,
                                available,
        ...
        if remaining:
            self.loginfo(
                "reinforce: randomly distributing %s remaining forces", remaining
            )
