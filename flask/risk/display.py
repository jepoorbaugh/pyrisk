...
class CursesDisplay(Display):
    ...
    def format(self, msg):
        if msg[0] == "start":
            ...
            _, player, t, f = msg
            return "%s reinforces %s with %d (total %d)" % (
                player.name,
                t.name,
                f,
                t.forces,
            ...
            return "%s moves %d from %s to %s (total %d)" % (
                player.name,
                f,
                st.name,
                tt.name,
    ...
    def update(self, msg, territory=None, player=None, extra=None, modal=False):
        ...
        for name, t in self.game.world.territories.items():
            ...
            if t.owner:
                self.worldpad.addstr(
                    self.t_centre[name][0], self.t_centre[name][1], str(t.forces), attrs
                )

        self.infopad.clear()
        info = (
            "TURN "
            + str(self.game.turn // len(self.game.players))
            + ": "
            + self.format(msg)
        )
        ...
        for i, name in enumerate(self.game.turn_order):
            ...
            info = [
                p.name,
                p.ai.__class__.__name__[:-2],
                str(self.game.options["history"].get(p.name, 0)),
                str(p.territory_count),
                str(p.forces),
                str(p.reinforcements),
            ]
            info = "".join(s.ljust(8)[:8] for s in info) + " ".join(
                a.name for a in p.areas
            )
            self.infopad.addstr(3 + i, 0, info, attrs)

        logline = len(self.game.turn_order) + 5
        for i, record in enumerate(self.logqueue.queue):
            self.infopad.addstr(
                logline + i, 0, record.getMessage()[: self.ix - 1], curses.A_NORMAL
            )
            if logline + i == self.iy - 2 and i < len(self.logqueue.queue) - 1:
                self.infopad.addstr(
                    logline + i + 1,
                    0,
                    "(%d more suppressed)" % (len(self.logqueue.queue) - i),
                    curses.A_NORMAL,
                )
                break
        ...
        self.worldpad.overwrite(
            self.screen,
            0,
            0,
            1,
            1,
            min(self.sy - 1, self.wy),
            min(self.sx - 1, self.wx - 1),
        )
        self.infopad.overwrite(
            self.screen,
            0,
            0,
            min(self.wy + 2, self.sy - 1),
            0,
            min(self.sy - 1, self.wy + self.iy + 1),
            min(self.ix - 1, self.sx - 1),
        )
        self.screen.refresh()
