...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable quiet mode so no logs will be outputted",
    )
    parser.add_argument(
        "-l",
        "--log",
    ...
    parser.add_argument(
        "-s", "--seed", type=int, default=None, help="Random number generator seed"
    )
    parser.add_argument(
        "-g", "--games", type=int, default=1, help="Number of rounds to play"
    )
    parser.add_argument(
        "-w",
        "--wait",
    ...
    if args.games == 1:
        ...
        for k in sorted(wins, key=lambda x: wins[x]):
            print(
                "%s [%s]:\t%s" % (k, player_classes[NAMES.index(k)].__name__, wins[k])
            )
